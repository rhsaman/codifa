...
function Ne(n) {
  ...
  try {
    o.writeFileSync(q(), JSON.stringify({ path: e }, null, 2), "utf-8");
  } catch {}
  return e;
}
function me(n, e) {
...
async function ge(n, e, t) {
  ...
  for (const s of r) {
    const a = c.join(n, s),
      l = c.join(e, s),
      d = await k.stat(a);
    if (d.isDirectory()) await ge(a, l, t);
    else if (d.isFile())
      try {
        await k.copyFile(a, l);
      } catch {}
    (i++,
      t(c.basename(n), Math.round((i / r.length) * 100)),
...
function We(n, e) {
  const t = D(),
    r = c.resolve(n.trim() || "");
  if (!r || r === t || r.startsWith(t + c.sep)) return Promise.resolve(t);
  const i = Oe(t);
  return (
    e("Preparing", 0),
    Me(t, r, i, e)
      .then(() => e("Cleaning up old location", 90))
      .then(() => He(t, i))
      .then(() => (e("Done", 100), r))
  );
}
...
async function He(n, e) {
  for (const t of e) {
    const r = c.join(n, t);
    try {
      (await k.stat(r)).isDirectory()
        ? await k.rm(r, { recursive: !0, force: !0 })
        : await k.unlink(r);
    } catch {}
...
function J(n) {
  const e = we();
  try {
    (o.mkdirSync(c.dirname(e), { recursive: !0 }),
      o.writeFileSync(e, n, { mode: 384 }));
    try {
      o.chmodSync(e, 384);
    } catch {}
  } catch {}
...
function Se() {
  ...
  if (o.existsSync(n))
    try {
      ...
      if (t.startsWith(U)) {
        ...
        L.isEncryptionAvailable() &&
          (e = L.decryptString(Buffer.from(r, "base64")));
  ...
  if (!e || e.length < 16) {
    ...
    try {
      L.isEncryptionAvailable()
        ? J(`${U}${L.encryptString(e).toString("base64")}`)
        : (console.warn(
            "[secrets] OS keychain unavailable (e.g. headless Linux) — storing the encryption key with file permissions only (0600) instead of safeStorage.",
          ),
          J(`${N}${e}`));
    } catch {
      (console.warn(
        "[secrets] safeStorage encrypt failed — storing the encryption key with file permissions only (0600) instead of safeStorage.",
      ),
        J(`${N}${e}`));
    }
  }
...
function Ue() {
  ...
  for (const e of n) if (e && o.existsSync(c.join(e, "server.py"))) return e;
  return null;
}
function Be(n) {
...
function Je() {
  ...
  if (n) return Array.from(t).join(e);
  for (const r of [
    "/opt/homebrew/bin",
    "/opt/homebrew/sbin",
    "/usr/local/bin",
    ...
    const r =
      ...
      i =
        process.platform === "darwin"
          ? `${r} -l -c 'echo -n "$PATH"'`
          : `${r} -c 'echo -n "$PATH"'`,
      s = Pe(i, { timeout: 3e3, encoding: "utf8" });
    ...
    (t.unref(),
      ...
      t.listen(0, "127.0.0.1", () => {
        ...
        if (r && typeof r == "object") {
          const i = r.port;
          t.close(() => n(i));
        } else t.close(() => e(new Error("could not allocate port")));
      }));
  });
}
let A = null,
...
async function Ge() {
  var l, d;
  const n = Ue();
  if (!n) throw new Error("backend/server.py not found; run `npm run setup`");
  const e = await qe(),
    t = `http://127.0.0.1:${e}`,
    r = Be(n);
  let i;
  ...
  ((l = i.stdout) == null ||
    ...
    i.on("exit", (u, p) => {
      (M
        ? (console.log(`[sidecar] stopped (${p || "exit"})`), (M = !1))
        : console.error(
            u === null
              ? `[sidecar] exited unexpectedly (signal ${p})`
              : `[sidecar] exited with code ${u}`,
          ),
        (A = null));
    }),
    i.on("error", (u) => {
  ...
  for (; Date.now() < a;) {
    if (i.exitCode !== null)
      throw new Error(
        "Python sidecar exited during startup; run `npm run setup`",
      );
    try {
  ...
  throw (
    i.kill(),
    new Error("Python sidecar did not become healthy; run `npm run setup`")
  );
}
async function T() {
  try {
...
const Ve = [
  ...
  Xe = /^\s*(?:export\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.*?)\s*$/;
function Ye(n) {
  const e = n.trim();
  if (e.length >= 2) {
    const t = e[0],
...
function H(n, e) {
  const t = o.realpathSync(n),
    r = e.replace(/\\/g, "/").trim().replace(/^\/+/, "");
  if (!r) return t;
  const i = c.join(t, r);
  let s = i;
  for (; !o.existsSync(s);) {
...
function it(n, e, t) {
  ...
  const i = c.dirname(r);
  (o.existsSync(i) || o.mkdirSync(i, { recursive: !0 }),
    o.writeFileSync(r, t, "utf-8"));
}
function st(n, e) {
  ...
  for (; t.length > 0;) {
    ...
    for (const s of i) {
      ...
      if (s.isDirectory()) {
        ...
        t.push(a);
      } else if (s.isFile() && (e.push(a), e.length >= at)) return e;
    }
  }
  return e;
}
...
function ft(n) {
  ...
  try {
    ...
    return t.length > 8 * 1024 * 1024
      ? null
      : `data:${e};base64,${t.toString("base64")}`;
  } catch {
    return null;
  }
}
...
function z(n) {
  const e = D(),
    t = n.replace(/\\/g, "/").trim().replace(/^\/+/, "");
  if (!t) return e;
  const r = c.resolve(e, t),
    i = o.existsSync(r) ? o.realpathSync(r) : r;
...
function fe(n, e) {
  try {
    const t = c.join(ht(), n);
    if (o.existsSync(t)) return JSON.parse(o.readFileSync(t, "utf-8"));
  } catch {}
  return e;
}
...
function wt() {
  ...
  return (
    t && n.push(c.join(t, "nvim")),
    n.push(c.join(x.homedir(), "Library", "Caches", "nvim")),
    n
  );
}
function St(n) {
  const e = c.basename(n),
    t = e.match(/^nvim\.(\d+)\.0$/);
...
async function ke() {
  const n = [];
  for (const t of wt()) t && (await xe(t, 0, n));
  const e = [...new Set(n)];
  return e.length > 0
    ? e
    : new Promise((t) => {
        X(
          ...
          (r, i) => {
            ...
            for (const a of i.split("\0")) {
              ...
              let l = a.slice(1);
              if (
                (l.startsWith("->") && (l = l.slice(2)),
                !!l.startsWith("/") && /nvim\.\d+\.0$/.test(l))
              )
                try {
    ...
    ((await ke()).length === 0
      ? ((I += 1),
        I > 0 &&
          ...
          (clearInterval(y),
          (y = setInterval(n, 5e3)),
          (t = y.unref) == null || t.call(y)))
      : I > 0 && y
        ? ((I = 0),
          clearInterval(y),
          (y = setInterval(n, 1500)),
          (r = y.unref) == null || r.call(y))
        : (I = 0),
      await xt());
  };
  setTimeout(() => {
...
function Ae() {
  ((b = new E({
    ...
    he && process.env.VITE_DEV_SERVER_URL
      ? b.loadURL(process.env.VITE_DEV_SERVER_URL)
      : b.loadFile(c.join(v.getAppPath(), "dist", "index.html")),
    b.on("closed", () => {
      b = null;
    }));
}
function At() {
  (ze(),
    f.handle("sidecar:url", async () => T()),
    f.handle("nvim:get", () => ({ abs: G, diagnostics: K })));
  const n = /^[A-Z][A-Z0-9_]*$/;
  (f.handle("env:get", (e, t) =>
    typeof t != "string" || t.length === 0 || t.length > 128 || !n.test(t)
      ? null
      : (process.env[t] ?? null),
  ),
    f.handle("oauth:google", async (e, t, r, i) => {
      const s = typeof t == "string" ? t.trim() : "",
        a = typeof r == "string" ? r.trim() : "";
      ...
      for (; Date.now() < F;) {
        ...
        const w = await fetch(
          `${l}/oauth/google/result?state=${encodeURIComponent(p)}`,
          {
    ...
    f.handle("fs:list", (e, t, r) => nt(t, r)),
    f.handle("fs:walk", (e, t) => tt(t)),
    f.handle("fs:read", (e, t, r) => rt(t, r)),
    f.handle("fs:write", (e, t, r, i) => (it(t, r, i), !0)),
    f.handle("fs:delete", (e, t, r) => st(t, r)),
    f.handle("fs:search", (e, t, r) => ut(t, r)),
    f.handle("fs:read-image", (e, t) => ft(t)),
    f.handle(
      "clipboard:write",
      (e, t) => (Fe.writeText(typeof t == "string" ? t : ""), !0),
    ),
    f.handle("coder:list", (e, t) => pt(t)),
    f.handle("coder:read", (e, t) => yt(t)),
    f.handle("coder:write", (e, t, r) => (mt(t, r), !0)),
    f.handle("coder:delete", (e, t) => gt(t)),
    f.handle("screenshot:capture", async () => {
      const e = ae.getPrimaryDisplay(),
        { width: t, height: r } = e.bounds,
        i = await ce.getSources({
          types: ["screen"],
          thumbnailSize: { width: t, height: r },
        }),
        s = i.find((d) => d.display_id === String(e.id)) ?? i[0];
      if (!s || s.thumbnail.isEmpty()) return null;
      const a = s.thumbnail.toPNG(),
        l = c.join(x.tmpdir(), `coder-shot-${Date.now()}.png`);
      try {
        o.writeFileSync(l, a);
      } catch {
        return null;
      ...
      return {
        path: l,
        dataUrl: `data:image/png;base64,${a.toString("base64")}`,
      };
    }),
    f.handle("screenshot:capture-region", async () => {
      const e = ae.getPrimaryDisplay(),
        t = e.scaleFactor || 1,
        { x: r, y: i, width: s, height: a } = e.bounds,
        l = await ce.getSources({
          types: ["screen"],
          thumbnailSize: {
            width: Math.round(s * t),
            height: Math.round(a * t),
          },
        }),
        d = l.find((F) => F.display_id === String(e.id)) ?? l[0];
      if (!d || d.thumbnail.isEmpty()) return null;
      const u = d.thumbnail,
        p = c.join(x.tmpdir(), `coder-overlay-${Date.now()}.html`);
      try {
      ...
      return await new Promise((F) => {
        ...
        (w.setAlwaysOnTop(!0, "screen-saver"),
          w.setVisibleOnAllWorkspaces(!0, { visibleOnFullScreen: !0 }));
        let _ = !1;
        const j = (C) => {
            if (!_) {
              ((_ = !0),
                f.removeListener("overlay:selected", ee),
                f.removeListener("overlay:cancel", te),
                clearTimeout(je));
              try {
                o.unlinkSync(p);
              } catch {}
          ...
          $ = (C, g) => Math.max(0, Math.min(Math.round(C), g)),
          ee = (C, g) => {
            const Ee = {
                x: $(((g == null ? void 0 : g.x) ?? 0) * t, u.getSize().width),
                y: $(((g == null ? void 0 : g.y) ?? 0) * t, u.getSize().height),
                width: $(
                  ((g == null ? void 0 : g.width) ?? 0) * t,
                  u.getSize().width,
                ),
                height: $(
                  ((g == null ? void 0 : g.height) ?? 0) * t,
                  u.getSize().height,
                ),
              },
              ne = u.crop(Ee);
            ...
            const re = ne.toPNG(),
              ie = c.join(x.tmpdir(), `coder-shot-${Date.now()}.png`);
            try {
            ...
            j({
              path: ie,
              dataUrl: `data:image/png;base64,${re.toString("base64")}`,
            });
          },
          te = () => j(null);
        (f.on("overlay:selected", ee),
          f.on("overlay:cancel", te),
          w.on("closed", () => j(null)));
    ...
    f.handle("image:normalize", (e, t) => {
      ...
      const { width: i, height: s } = r.getSize(),
        a = 2048;
      if (i > a || s > a) {
      ...
      const l = r.toPNG(),
        d = c.join(
          x.tmpdir(),
          `coder-img-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.png`,
        );
      try {
        o.writeFileSync(d, l);
      } catch {
      ...
      return {
        path: d,
        dataUrl: `data:image/png;base64,${l.toString("base64")}`,
      };
    }),
    ...
    f.handle("store:set", (e, t, r) =>
      t === "settings"
        ? (O({ settings: r }), !0)
        : t === "chats"
          ? (O({ chats: Array.isArray(r) ? r : [] }), !0)
          : t === "deleted_chats"
            ? (O({ deleted_chats: Array.isArray(r) ? r : [] }), !0)
            : t === "deleted_workspaces"
              ? (O({ deleted_workspaces: Array.isArray(r) ? r : [] }), !0)
              : !1,
    ),
    f.handle("data:path", () => D()),
    f.handle("data:has-settings", () => {
      try {
    ...
    f.handle("data:move", async (e, t) => {
      (await Q(), await _e());
      const r = E.getAllWindows()[0],
        i = await We(t, (s, a) => {
          r &&
            !r.isDestroyed() &&
            r.webContents.send("migrate:progress", { label: s, pct: a });
        });
      Ne(i);
      try {
        await ve();
      ...
      for (const s of E.getAllWindows()) s.webContents.send("sidecar:changed");
      return ((m = null), i);
    }));
}
let m = null,
...
async function jt() {
  ...
  if (n)
    try {
      const e = await fetch(`${n}/app/state`),
        t = e.ok ? await e.json() : {},
        r = V(),
        i = t.settings ?? (r == null ? void 0 : r.settings) ?? null,
        s =
          Array.isArray(t.chats) && t.chats.length > 0
            ? t.chats
            : ((r == null ? void 0 : r.chats) ?? []);
  ...
  if (n)
    try {
      ...
      ((m = {
        ...
        Z(m));
      for (const i of E.getAllWindows()) i.webContents.send("sidecar:changed");
    } catch {}
}
function O(n) {
  (n.settings !== void 0 && (h.settings = n.settings),
    n.chats !== void 0 && (h.chats = n.chats),
    n.deleted_chats !== void 0 &&
      (h.deleted_chats = [...(h.deleted_chats ?? []), ...n.deleted_chats]),
    n.deleted_workspaces !== void 0 &&
      (h.deleted_workspaces = [
        ...(h.deleted_workspaces ?? []),
        ...n.deleted_workspaces,
      ]),
    P && clearTimeout(P),
    (P = setTimeout(() => void Q(), 300)));
}
async function Q() {
  var t, r, i, s;
  if (
    (W &&
      (await W,
      !h.settings &&
        !Array.isArray(h.chats) &&
        !((t = h.deleted_chats) != null && t.length) &&
        !((r = h.deleted_workspaces) != null && r.length))) ||
    (P && (clearTimeout(P), (P = null)),
    !h.settings &&
      !Array.isArray(h.chats) &&
      !((i = h.deleted_chats) != null && i.length) &&
      !((s = h.deleted_workspaces) != null && s.length))
  )
    return;
  const n = h;
  h = {};
  const e = (async () => {
    ...
    try {
      ...
      m &&
        (n.settings !== void 0 && (m.settings = n.settings),
        Array.isArray(n.chats) && (m.chats = n.chats),
        (l = n.deleted_chats) != null &&
          l.length &&
          (m.chats = (m.chats ?? []).filter(
            (p) => !n.deleted_chats.includes(p.id),
          )),
        Z(m));
    } catch {
      pe(n);
...
function pe(n) {
  var e, t, r, i;
  (n.settings !== void 0 && h.settings === void 0 && (h.settings = n.settings),
    Array.isArray(n.chats) && !Array.isArray(h.chats) && (h.chats = n.chats),
    (e = n.deleted_chats) != null &&
      e.length &&
      !((t = h.deleted_chats) != null && t.length) &&
      (h.deleted_chats = [...(h.deleted_chats ?? []), ...n.deleted_chats]),
    (r = n.deleted_workspaces) != null &&
      r.length &&
      !((i = h.deleted_workspaces) != null && i.length) &&
      (h.deleted_workspaces = [
        ...(h.deleted_workspaces ?? []),
        ...n.deleted_workspaces,
      ]));
...
async function Dt() {
  ...
  for (const e of n) await Ft(e);
}
async function Ft(n) {
  const e = c.join(n, "settings.json"),
    t = c.join(n, "chats.json"),
    r = o.existsSync(e),
    i = o.existsSync(t);
  if (!r && !i) return;
...
v.whenReady().then(async () => {
  (At(),
    se.defaultSession.setPermissionRequestHandler((n, e, t) => {
  ...
  try {
    await Promise.race([Dt(), new Promise((n) => setTimeout(n, 4e3))]);
  } catch (n) {
      ...
      if (!i) {
        ...
        for (
          ...
          !!(
            h.settings !== void 0 ||
            Array.isArray(h.chats) ||
            (((l = h.deleted_chats) == null ? void 0 : l.length) ?? 0) > 0 ||
            (((d = h.deleted_workspaces) == null ? void 0 : d.length) ?? 0) > 0
          ));
          u++
        )
