...
> **نکته محیط نصب:** در این اپ، اسکیلها فقط در دیتابیس ذخیره میشوند و اسکریپت جستجوی محلی (`search.py`) و فایلهای `references/` روی دیسک موجود نیستند. پس اگر اسکریپت در دسترس نبود، بهجای آن از جدول اولویت ۱→۱۰ بالا و راهنمای داخلی همین متن استفاده کن و به کاربر بگو نتیجه از دیفالتهای داخلی آمده است.
