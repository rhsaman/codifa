...
EXPLORE_SYSTEM = (
    ...
    "- Fire multiple Glob/Grep calls in the SAME turn (parallel tool calls) when "
    "you already know the searches you need; combine alternatives with `foo|bar` "
    "to collapse several searches into one. Avoid tiny repeated 30-line read "
    "slices — if you need more context, read a larger window (limit=300+) instead "
    "of paging 30 lines at a time\n"
    "- To discover files use Glob, not read on a directory. Only read specific files you have located; "
    "never list a directory with read — it duplicates what Glob already gives you\n"
